...
void SwerveModule::UpdateSimulation(units::second_t period) {
    // Simulate drive encoder
    double driveVoltage = m_driveMotor.Get() * 12.0; // Assuming 12V max
    double driveSpeed = driveVoltage / 12.0 * kMaxSpeed.value(); // Scale to max speed
    m_simDriveRate = driveSpeed;
    m_simDriveDistance += driveSpeed * period.value();

    // Simulate turning encoder
    double turnVoltage = m_turningMotor.Get() * 12.0; // Assuming 12V max
    double turnSpeed = turnVoltage / 12.0 * kMaxTurnSpeed.value(); // Scale to max turn speed
    m_simTurnDistance += turnSpeed * period.value();
}
